...
func getFunctionComment(handlerValue reflect.Value, handlerType reflect.Type) string {
	...
	return p.findCommentInSource(methodName)
}

// extractCommentForFunction extract comment for a specific function
...
func getHandlerResponseStatus(handlerValue reflect.Value, handlerType reflect.Type) int {
	...
	// Fallback to scanning all files
	status := p.findStatusInSource(methodName)
	if status != 0 {
		return status
	}
...
// findFiles finds all Go files in a directory structure
func (p *sourceFileParser) findFiles(maxDepth int) ([]string, error) {
	...
	err = filepath.Walk(currentDir, walkFn)
	return files, err
}

// findCommentInSource looks for comments in source files
func (p *sourceFileParser) findCommentInSource(methodName string) string {
	files, err := p.findFiles(2) // Look 2 levels deep
	if err != nil {
		return ""
	}
...
// findStatusInSource looks for status codes in source files
func (p *sourceFileParser) findStatusInSource(methodName string) int {
	files, err := p.findFiles(2) // Look 2 levels deep
	if err != nil {
		return 0
	}
